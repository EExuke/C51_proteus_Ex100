//-----------------------------------------------------------------
//  名称: 1-Wire总线器件ROM搜索及多点DS18B20监测程序
//-----------------------------------------------------------------
#include <reg51.h>
#include <intrins.h>
#include <string.h>
#include <stdio.h>
#define INT8	signed char
#define INT8U	unsigned char
#define INT16U	unsigned int
#include "1-Wire.h"
INT8U	Temp_Value[2];				//存放所读取的两字节温度数据
INT8U	ROMCODE[8];					//ROMCODE(64位)
INT8U	LCD_Buffer[21];				//液晶显示缓冲
//当前分支层及上次搜索的最低分支层
INT8	Level,Last_Level;			//(注意将二者定义为有符号数)
INT8U	CRC8;						//CRC-8校验变量
bit		bit_A,bit_B, bj;			//读ROMCODE的当前位原码与反码等
extern	delay_ms(INT16U);
extern	void LCD_ShowString(INT8U r, INT8U c,INT8U *str);
//-----------------------------------------------------------------
// 延时nus
//-----------------------------------------------------------------
void delay_us(INT8U x){	x >>= 1; while (--x);}
//-----------------------------------------------------------------
// 延时nx100ms
//-----------------------------------------------------------------
void delayX100ms(INT8U n){	while (n--) delay_ms(100);}
//-----------------------------------------------------------------
// 发送读时隙时序
//-----------------------------------------------------------------
void Read_Slot(){_nop_(); DQ = 0; _nop_(); DQ = 1; _nop_();}
//-----------------------------------------------------------------
// 从1-Wire总线读取1位
//-----------------------------------------------------------------
INT8U Read_Romcode_bit()
{

}

//-----------------------------------------------------------------
// 向1-Wire总线写1字节
//-----------------------------------------------------------------
void Write_Byte(INT8U A)
{


}

//-----------------------------------------------------------------
// 从1-Wire总线读取1字节
//-----------------------------------------------------------------
INT8U Read_Byte()
{	 


}

//-----------------------------------------------------------------
// 向1-Wire总线写1位0/1
//-----------------------------------------------------------------
void Write_DQ_bit(INT8U b)
{


} 

//-----------------------------------------------------------------
// 1-Wire总线复位
//----------------------------------------------------------------- 
INT8U RESET()
{	 


}
//-----------------------------------------------------------------
// 转换并显示当前找到的器件的8字节(64位)ROMCODE
//-----------------------------------------------------------------
void Show_Romcode(INT8U r, INT8U c)
{


}

//-----------------------------------------------------------------
// 搜索1-Wire总线上一只器件的64位ROMCODE
// 返回0表示搜索到一个ROMCODE,否则表示无器件或搜索的ROMCODE校验错
//-----------------------------------------------------------------
INT8U Search_ROM1()
{



}

//-----------------------------------------------------------------
// 搜索1-Wire上挂载的所有器件的ROMCODE
//-----------------------------------------------------------------
void Search_ALL_ROM()
{ 



}

//-----------------------------------------------------------------
// 读取存放于8字节数组A中的ROMCODE的第i位(0/1)
//-----------------------------------------------------------------
INT8U Read_ROMCODE_Bit_i(INT8U A[],INT8U i)
{



}

//-----------------------------------------------------------------
// 将存放于8字节数组A中的ROMCODE的第i位设为0/1
//-----------------------------------------------------------------
void Save_ROMCODE_Bit_i(INT8U A[],INT8U i,INT8U b)
{	 


}

//-----------------------------------------------------------------
// 根据ROMCODE码读取温度数据
//-----------------------------------------------------------------
float Get_Temperature(char *rom_code)
{	


}

//-----------------------------------------------------------------
// 发送匹配命令,并发送64位的ROMCODE
//-----------------------------------------------------------------
void ROMCODE_Match(INT8U ROMCODE[])
{	 


}

//-----------------------------------------------------------------
// 基于比特的CRC_8校验函数 （速度较慢，本例未调用）
// (通过该函数可预先得出256字节的校验码表,改用查表法进行校验)
// 校验多项式: x^8 + x ^ 5 + x ^ 4 + 1, 去高位后倒序:0x8C
//-----------------------------------------------------------------
/*void CRC_8(INT8U d)	
{


}*/

//-----------------------------------------------------------------
// CRC_8校验码表(共256字节，该码表通过TC程序调用上述函数计算得到)
//-----------------------------------------------------------------
code INT8U CRC_T8[] = 	
{	
};
//-----------------------------------------------------------------
// CRC_8查表校验函数(生成多项式为: x^8 + x ^ 5 + x ^ 4 + 1)
//-----------------------------------------------------------------
void CRC_8(INT8U d) {                   }